...
Shader "Custom/GameBroadcast/PartialTransparencyShader" {
	Properties {
		...
		_DistanceThreshold ("Distance Threshold", Float) = 0.5
	...
	SubShader {
		...
		Pass {
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
				...
				float3 camToTarget = normalize(_TargetPos - _CameraPos);

				// Проекция позиции пикселя на направление камеры к цели
				float t = dot(IN.worldPos - _CameraPos, camToTarget);

				// Ближайшая точка на линии камеры-цели к текущей позиции пикселя
				float3 closestPoint = _CameraPos + camToTarget * t;

				// Расстояние от текущей позиции пикселя до этой точки
				float dist = distance(IN.worldPos, closestPoint);

				// Определяем альфа-прозрачность на основе расстояния
				float alpha = _Transparency;
				if (dist < _DistanceThreshold)
				{
					alpha = lerp(_Transparency, 0.0, 1.0 - smoothstep(0.0, _DistanceThreshold, dist));
				}
